import { LocaleStrings } from '../index';

const es: LocaleStrings = {
  historyTooltip: 'Historial',
  settingsTooltip: 'Configuración',
  translateButton: 'Traducir',
  translatingButton: 'Traduciendo...',
  captureTooltip: 'Capturar pantalla',
  restartButton: 'Reiniciar',
  downloadingUpdate: 'Descargando…',
  updateFailed: 'Error al buscar actualizaciones — toca para reintentar',
  trayCapture: 'Capturar pantalla',
  trayOpen: 'Abrir',
  trayQuit: 'Salir',

  welcome: 'Bienvenido a Screen Translator',
  welcomeSubtitle: 'Para comenzar, ingresa tu clave API de Gemini a continuación. Se usa para el reconocimiento y la traducción de texto.',
  settings: 'Configuración',
  apiKeyRequired: 'La clave API es obligatoria',
  geminiApiKey: 'Clave API de Gemini',
  apiKeyPlaceholder: 'Ingresa tu clave API de Gemini',
  getApiKeyLink: 'Obtener una clave API de Gemini →',
  globalHotkey: 'Atajo de teclado global',
  hotkeyPlaceholder: 'Haz clic aquí y luego presiona el atajo',
  hotkeyHelp: 'Haz clic en el campo y presiona la combinación de teclas deseada',
  model: 'Modelo',
  loadingModel: 'Cargando o no disponible',
  modelRecommended: 'Recomendado',
  appLanguageLabel: 'Idioma de la app',
  themeLabel: 'Tema',
  themeLight: 'Claro',
  themeDark: 'Oscuro',
  themeSystem: 'Sistema',
  alwaysOnTopLabel: 'Siempre visible',
  launchAtStartupLabel: 'Iniciar con el sistema',
  startMinimizedToTrayLabel: 'Iniciar minimizado en la bandeja',
  startMinimizedToTrayHelp: 'Solo se aplica cuando "Iniciar con el sistema" está activado',
  settingsTabGeneral: 'General',
  settingsTabAppearance: 'Apariencia',
  settingsTabStartup: 'Inicio',
  cancel: 'Cancelar',
  save: 'Guardar',
  getStarted: 'Comenzar',

  permissionTitle: 'Permiso de grabación de pantalla',
  permissionDescription: 'Screen Translator necesita permiso para grabar tu pantalla y capturar texto.',
  permissionMacStep1: 'Haz clic en Abrir configuración abajo',
  permissionMacStep2: 'Encuentra Screen Translator en la lista',
  permissionMacStep3: 'Activa el interruptor junto a él',
  permissionMacStep4: 'Reinicia la app',
  permissionWinStep1: 'Haz clic en Abrir configuración abajo',
  permissionWinStep2: 'Activa la grabación de pantalla para esta app',
  permissionWinStep3: 'Reinicia la app',
  permissionGeneric: 'Por favor, otorga permiso de grabación de pantalla en la configuración del sistema y reinicia la app.',
  close: 'Cerrar',
  openSettings: 'Abrir configuración',

  accessibilityPermissionTitle: 'Permiso de Accesibilidad Requerido',
  accessibilityPermissionDescription: 'Screen Translator necesita permiso de Accesibilidad para registrar el atajo de teclado global. Sin él, el atajo no funcionará.',
  accessibilityPermissionStep1: 'Haz clic en Abrir Configuración a continuación',
  accessibilityPermissionStep2: 'Encuentra Screen Translator en la lista de Accesibilidad y activa el interruptor',
  accessibilityPermissionStep3: 'Reinicia la aplicación',
  accessibilityWarningTooltip: 'Atajo no disponible — haz clic para otorgar permiso de Accesibilidad',

  recentHistory: 'Historial reciente',
  noHistory: 'Sin historial todavía',
  clearHistory: 'Borrar todo el historial',
  today: 'Hoy',
  yesterday: 'Ayer',
  deleteHistoryEntry: 'Eliminar',

  sourceText: 'Texto fuente',
  paste: 'PEGAR',
  copy: 'Copiar',
  clearText: 'Borrar texto',
  typePlaceholder: 'Escribe o pega para traducir...',
  pasteFailed: 'No se pudo pegar desde el portapapeles',
  captureStartFailed: 'No se pudo iniciar la captura de pantalla',

  translation: 'Traducción',
  altToggleLabel: 'Alt.',
  contextToggleLabel: 'Contexto',
  alternatives: 'Alternativas',
  contextOfUse: 'Contexto de uso',

  expandPanelTooltip: 'Expandir',
  restoreSplitViewTooltip: 'Restaurar vista dividida',

  apiKeyNotSet: 'Clave API no configurada. Por favor, establece tu clave API de Gemini en la configuración.',

  whatsNewTitle: "Novedades en v{version}",
  whatsNewClose: 'Entendido',
  updateAvailableTitle: 'Actualización disponible',
  updateAvailableMessage: 'Hay una nueva versión de Screen Translator disponible: v{version}.',
  updateNow: 'Actualizar',
  ignoreRelease: 'Ignorar esta versión',

  languageNames: {
    'Auto': 'Auto',
    'English': 'Inglés',
    'Russian': 'Ruso',
    'Ukrainian': 'Ucraniano',
    'Spanish': 'Español',
    'French': 'Francés',
    'German': 'Alemán',
    'Italian': 'Italiano',
    'Portuguese': 'Portugués',
    'Chinese (Simplified)': 'Chino (simplificado)',
    'Japanese': 'Japonés',
    'Korean': 'Coreano',
    'Polish': 'Polaco',
  },

  capture: {
    instruction: 'Haz clic y arrastra para seleccionar un área. Pulsa ESC para cancelar.',
    processing: 'Procesando captura...',
    initializing: 'Inicializando captura de pantalla...',
    close: 'Cerrar',
  },

  notification: 'Notificación',
  ok: 'Aceptar',
  modelUnavailableTitle: 'Modelo no disponible',
  modelUnavailableMessage: 'El modelo {oldModel} no está disponible. Se cambió a {newModel}.',

  welcomeAnalyticsTitle: 'Ayúdanos a mejorar Screen Translator',
  welcomeAnalyticsIntro: 'Antes de configurar tu clave de API, dinos si estás de acuerdo en compartir datos de uso anónimos.',
  analyticsUpdateTitle: 'Antes de continuar',
  analyticsUpdateIntro: 'Nos gustaría recopilar datos de uso anónimos para mejorar Screen Translator.',
  legalDocsUpdatedTitle: 'Nuestra política ha cambiado',
  legalDocsUpdatedIntro: 'Hemos actualizado nuestra Política de Privacidad y Términos de Uso. Tómate un momento para revisarlos.',
  analyticsConsentLabel: 'Compartir datos de uso anónimos',
  analyticsConsentDescription: 'Nos ayuda a entender cuántas personas usan la aplicación y con qué frecuencia. Nunca se recopila el contenido de la pantalla, las traducciones ni datos personales.',
  privacyPolicyLink: 'Política de Privacidad',
  termsOfUseLink: 'Términos de Uso',
  welcomeModalContinue: 'Continuar',
};

export default es;